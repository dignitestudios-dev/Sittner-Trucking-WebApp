          ...
          <tbody>
            {
              Notification?.map((item,i)=>(
                ...
                <td className="px-6 py-10">
                  <div className={` ${item.status=="Scheduled"?"bg-[#FF99001F] text-[#FF9900]":"text-[#41C54E] bg-[#41C54E26]"}  w-[100px] text-center  text-xs font-bold me-2 px-4 py-2 rounded-full`}>
                    {item.status=="Scheduled"?"Scheduled":"Delivered"}
                  </div>
                </td>
                <td className="px-6 py-10">
                  <NavLink to={"/editnotification"} state={{data:item}} className="bg-transparent">
                    <img
                      src="/whiteedit.png"
