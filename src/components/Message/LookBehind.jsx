    ...
    <div className=" mt-3 h-[80%] scroll-box px-3  overflow-auto">
      ...
      deliveredNotifications.map((item,i)=>(
        ...
        <div className="username mb-3">
          <h2 className="font-semibold text-sm leading-[14px] ">   { i==0&&(new Date(item.date).toLocaleDateString("en-US", {
            weekday: "long",
          }))}</h2>
        </div>
        <div className="w-full py-2 px-2 bg-[#F9F9F9] border border-[#DFDFDF] rounded-[10px]">
          <h3 className="font-semibold text-sm leading-[14px]">Day {new Date(item.date).getDay()}</h3>
